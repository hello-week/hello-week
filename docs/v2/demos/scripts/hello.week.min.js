...
class HelloWeek {
    constructor(options) {
        ...
        this.calendar.navigation = createHTMLElement(this.selector, CSS_CLASSES.NAVIGATION, this.selector);
        if (this.options.nav) {
            const [prev, next] = this.options.nav;
            this.calendar.prevMonth = createHTMLElement(this.selector, CSS_CLASSES.PREV, this.calendar.navigation, prev);
            this.calendar.period = createHTMLElement(this.selector, CSS_CLASSES.PERIOD, this.calendar.navigation);
            this.calendar.nextMonth = createHTMLElement(this.selector, CSS_CLASSES.NEXT, this.calendar.navigation, next);
        ...
        this.calendar.week = createHTMLElement(this.selector, CSS_CLASSES.WEEK, this.selector);
        this.calendar.month = createHTMLElement(this.selector, CSS_CLASSES.MONTH, this.selector);
        if (this.options.rtl) {
    ...
    selectDay(callback) {
        this.daysOfMonth = this.selector.querySelectorAll('.' + CSS_CLASSES.MONTH + ' .' + CSS_CLASSES.DAY);
        ...
        target.addEventListener('click', (event) => {
            ...
            if (this.options.range) {
                ...
                if (this.intervalRange.begin && !this.intervalRange.end) {
                    ...
                    addClass(event.target, CSS_STATES.IS_END_RANGE);
                    if (this.intervalRange.begin > this.intervalRange.end) {
                        this.intervalRange.begin = undefined;
                        this.intervalRange.end = undefined;
                        this.removeStatesClass();
            ...
            for (let i = 1; i <= Object.keys(this.days).length; i++) {
                ...
                if (this.days[index].timestamp >= this.intervalRange.begin) {
                    if (this.days[i].timestamp >= this.intervalRange.begin &&
                        ...
                        addClass(this.days[i].element, CSS_STATES.IS_SELECTED);
                        if (this.days[i].timestamp === this.intervalRange.begin) {
                            addClass(this.days[i].element, CSS_STATES.IS_BEGIN_RANGE);
                        }
                    }
                }
            }
    ...
    createDay(date) {
        ...
        if (dayOptions.day === 1) {
            if (this.options.weekStart === DAYS_WEEK.SUNDAY) {
                setStyle(newDay, this.options.rtl ? 'margin-right' : 'margin-left', day * (100 / Object.keys(DAYS_WEEK).length) + '%');
            }
            else {
                if (day === DAYS_WEEK.SUNDAY) {
                    setStyle(newDay, this.options.rtl ? 'margin-right' : 'margin-left', (Object.keys(DAYS_WEEK).length -
