...
export function build(options: Options, args: any) {
  ...
  if (!isDef(self.selector)) {
    self.selector = render(
      h('div', { class: `${options.selector} ${cssClasses.CALENDAR}` })
    )
  } else {
  ...
  if (isDef(options.nav[0])) {
    self.calendar.prevMonth = render(
      h('div', { class: cssClasses.PREV }, options.nav[0]),
  ...
  if (isDef(options.nav[1])) {
    self.calendar.nextMonth = render(
      h('div', { class: cssClasses.NEXT }, options.nav[1]),
      self.calendar.navigation
    )
