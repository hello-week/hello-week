...
import { formatDate, setToTimestamp } from '../utils/format';
import { isArray, isDate, isNull, isString } from '../utils/is';
...
export class HelloWeek {
    ...
    constructor(options: Options) {
        ...
        this.selector = isString(this.options.selector)
            ...
            : (this.options.selector as unknown as HTMLElement);
    ...
    public destroy(options?: { removeElement?: boolean }): void {
        ...
        if (options?.removeElement) this.selector.remove();
    }
    ...
    public goToDate(date: Date | string | number = this.todayDate): void {
        ...
        if (isSameMonthAndYear(this.date, target)) return;
        this.date = target;
        this.date.setDate(1);
        this.forceUpdate();
    }
    ...
    /**
     * Get all days selected Date format.
     *
     * @see {@link Options}
     ...
     * @experimental
     */
    public getDaysSelected(): DateString[] {
        return this.daysSelected.map((day) =>
            formatDate(day.getDay(), day.getMonth(), day.getFullYear())
        );
    }
     ...
     *
     * @returns last day selected in timestamp.
     ...
     * @returns last day selected in timestamp.
     */
    public getLastDaySelected(): Date {
        return this.lastSelectedDay;
    }
    ...
    private getIntervalOfDates(startDate: Date, endDate: Date) {
        const dates = [];
        ...
        while (
            isDateAfter(currentDate, endDate) ||
            ...
            if (this.options.multiplePick) {
                if (this.days[index].timestamp) {
                    this.daysSelected = this.daysSelected.filter(
                        (day) => !isSameDate(day, this.lastSelectedDay)
                    );
                }
    ...
    private onHandleMouse(event: MouseEvent): void {
        ...
        for (let i = 1; i <= Object.keys(this.days).length; i++) {
            ...
            if (
                isDateAfter(this.days[index].date, this.intervalRange.begin) ||
                ...
                if (
                    ...
                    if (
                        isSameDate(this.days[i].date, this.intervalRange.begin)
                    ) {
                        addClass(
                            this.days[i].element,
    ...
    private createDay(date: Date): void {
        ...
        if (dayOptions.day === 1) {
            if (this.options.weekStart === HelloWeek.daysWeek.SUNDAY) {
                setStyle(
                    ...
                    dayOfWeek * (100 / Object.keys(HelloWeek.daysWeek).length) +
                        '%'
                );
            } else {
        ...
        if (
            dayOfWeek === HelloWeek.daysWeek.SUNDAY ||
