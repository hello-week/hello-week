import { CalendarOptions, CallbackFunction, DayOptions, DaysHighlight, IntervalRange, Langs, Options } from '../types';
import { addClass, creatHTMLElement, removeClass, setDataAttr, setStyle, toggleClass } from '../utils/dom';
...
export class HelloWeek {
    ...
    private setDaysDisable(newDay: HTMLElement, dayOptions: DayOptions): void {
        ...
        if (isArray(this.options.disableDates)) {
            this.options.disableDates.forEach((date) => {
                if (isString(date) && dayOptions.timestamp === setToTimestamp(date as string)) {
                    addClass(newDay, HelloWeek.cssStates.IS_DISABLED);
                    dayOptions.locked = true;
                }
            });
    ...
    private setStyleDayHighlight(newDay: HTMLElement, key: number, dayOptions: DayOptions) {
