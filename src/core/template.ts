...
export function template(options: IOptions, args: any): ITemplate {
    ...
    if (!isDef(self.selector)) {
        self.selector = render(
            el('div', { class: [options.selector, cssClasses.CALENDAR] })
        );
    } else {
    ...
    if (!isDef(self.calendar.navigation)) {
        self.calendar.navigation = render(
            el('div', { class: cssClasses.NAVIGATION }),
            self.selector
        );
    }
    ...
    if (isDef(options.nav[0])) {
        self.calendar.prevMonth = render(
            el('div', { class: cssClasses.PREV }, options.nav[0]),
    ...
    if (!isDef(self.calendar.period)) {
        self.calendar.period = render(
            el('div', { class: cssClasses.PERIOD }),
            self.calendar.navigation
        );
    }
    ...
    if (isDef(options.nav[1])) {
        self.calendar.nextMonth = render(
            el('div', { class: cssClasses.NEXT }, options.nav[1]),
            self.calendar.navigation
        );
